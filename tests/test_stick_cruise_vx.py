"""Stick is engage-only: depth must not change SoftTrot cruise."""
...
def test_stick_depth_maps_to_fixed_cruise_vx():
    fsm = _fsm()
    assert fsm._stick_cruise_vx(0.2) == 0.5
    assert fsm._stick_cruise_vx(1.0) == 0.5
    assert fsm._stick_cruise_vx(-0.8) == -0.5
...
def test_natural_stick_half_and_full_same_schedule():
    ...
    assert th_half == th_full == 0.5
    assert abs(amp_half - amp_full) < 1e-9
    assert abs(period_half - period_full) < 1e-9

    expect = SoftTrotSchedule(fsm._nat_schedule.env).map(VelocityCommand(vx=0.5))
